...
#define MAXLINE	1000	/* max size of input for ungets() */
...
int main()
{
	...
	char line[MAXLINE];	/* input line for ungets() */
	double variable[26];

	fgets(line, MAXLINE, stdin);	/* get line from user */
	ungets(line);			/* push line to buffer */
...
/* ungets: push back string onto input buffer */
/* we do not need to handle (buf[]) or (bufp).
 * we will use ungetch(), which handles them
 * for us */
void ungets(char s[])
{
	int i = strlen(s) - 1;
	while (i >= 0)
		ungetch(s[i--]); /* read into buffer backwards, getch() returns right to left */
}
